...
//
//
var _default = { data: function data() {return { imgUrl: 'https://sinomach.wxzhixun.com/images/', //图片接口
      curHdIndex: '0', orderId: '', orderList: [], type: 1 // orderList:[]
    };}, onShow: function onShow() {// sessionStorage.setItem('aaa', '123123order');
    this.curHdIndex = '0';_public.default.delOrderType = true;this.getOrderList(1);}, onLoad: function onLoad(e) {this.token = sessionStorage.getItem('token');}, methods: { //删除订单
    delItem: function delItem(theId, index) {var that = this;uni.showModal({ title: '提示', content: '确定删除该订单?', success: function success(res) {if (res.confirm) {uni.request({ url: that.nowUrl + '/foods/shopcar/remove?id=' + theId, header: { 'token': that.token }, method: 'POST', success: function success(res) {// console.log(res)
                // uni.navigateTo({
                // 	url: '../index/index'
                // })
                if (res.data.code == 200) {// that.getOrderList(1);
                  uni.showToast({ title: '删除成功', duration: 1500 });that.orderList.splice(index, 1);} else {uni.showToast({ icon: "none", title: res.data.msg, duration: 1500 });} // uni.navigateTo({
                // 	url: '../index/index',
